...
### 追加
- 文書評価機能：生成された医療文書の自動評価システム
  - `services/evaluation_service.py`：評価サービスのコアロジック
  - `external_service/gemini_evaluation.py`：Gemini APIクライアントによる評価実行
  - `views/evaluation_settings_page.py`：評価プロンプト設定UI
  - データベーステーブル`evaluation_prompts`：文書種別ごとの評価プロンプト管理
- 評価機能のUI要素：
  - 評価進捗表示と処理時間の表示機能
  - 評価完了時のメッセージ通知
  - ナビゲーションに「出力評価設定」ボタンを追加
  - メインページに「出力評価」ボタンと結果表示を追加
- テスト拡張：`tests/test_evaluation_service.py`で評価サービスのテストケースを追加

### 変更
- コードブロックの表示改善：折り返しと改行を適切に処理
- テキスト処理の改善：セクション検出ロジックとパターン正規化を強化
- 評価プロンプト管理：文書種別ごとに評価プロンプトを設定・管理可能に
- 設定ファイル整理：不要なバックアップセクションを削除

### 修正
- `.gitignore`のリファクタリング：`.env`以外の不要なファイルを削除
